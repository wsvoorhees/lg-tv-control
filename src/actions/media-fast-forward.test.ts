...
describe("MediaFastForward", () => {
    ...
    describe("onKeyDown", () => {
        ...
        it("sends fastForward request after connecting from disconnected", async () => {
            mockTvClient.waitForConnected.mockResolvedValue(undefined);
            mockTvClient.request.mockResolvedValue(undefined);
            await action.onKeyDown({} as never);
            expect(mockTvClient.wakeOnLan).toHaveBeenCalled();
            expect(mockTvClient.reconnect).toHaveBeenCalled();
            expect(mockTvClient.request).toHaveBeenCalledWith("ssap://media.controls/fastForward");
        });

        it("sends fastForward request when connected", async () => {
